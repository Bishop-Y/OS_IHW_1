...
int main(int argc, char *argv[]) {
    ...
    if (process_1 < 0) {
        ...
        if (process_2 < 0) {
            ...
            if ((fd_23 = open(second_channel, O_WRONLY)) < 0) {
                printf("Can\'t open FIFO for writing\n");
                exit(-1);
            }
            ...
            if ((fd_23 = open(second_channel, O_RDONLY)) < 0) {
                printf("Can\'t open FIFO for reading\n");
                exit(-1);
            }
            ...
            size = write(output, string, strlen(string)); // записываем в файл
